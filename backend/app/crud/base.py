...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def get(self, db: Session, id: Any) -> Optional[ModelType]:
        # Add explicit filter for soft deletes to ensure correctness.
        return db.query(self.model).filter(self.model.id == id, self.model.is_deleted == False).first()

    def get_multi(
        self, db: Session, *, skip: int = 0, limit: int = 100
    ) -> List[ModelType]:
        # Add explicit filter for soft deletes.
    ...
    def soft_remove(self, db: Session, *, db_obj: ModelType) -> ModelType:
        db_obj.soft_delete()
        db.commit()
        return db_obj